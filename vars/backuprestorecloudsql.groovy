def call(Map param) {
  if(param.backup=="true") {
   ...
   if(param.restore=="true") {
      ...
      sh "echo \"Y\" | gcloud sql backups restore \$(gcloud sql backups list --instance=${param.instance} --project ${param.project} | grep \"SUCCESSFUL\" | awk 'NR==1{print \$1}') --restore-instance=${param.project} --backup-instance=${param.instance}"
   }
}
