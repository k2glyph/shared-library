def call(Map param) {
  if(param.check=="jobs") {
    ...
    def result=sh(returnStdout: true, script: "kubectl -n ${param.namespace} wait --for=condition=complete --timeout=240s jobs/\$(kubectl get jobs --sort-by=.metadata.creationTimestamp -n ${param.namespace} | tail -1 | awk '{print \$1}') | cut -d\" \" -f2,3").trim()
